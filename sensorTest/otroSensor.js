...
const logger = winston.createLogger({
    ...
    transports: [
        ...
        new winston.transports.File({ filename: 'otroSensor.log' })
    ]
});
...
async function sendPulse() {
    ...
    const pulse = {
        ...
        valor: (Math.random() * 1000).toFixed(2), // Simulación de un valor diferente
        sensor: 'otroSensor.js',
        status: 'significant_event',
        timestamp: Date.now()
    };
...
async function handleCommand(command) {
    try {
        logger.info({ message: `Comando recibido: ${command}`, sensor: 'otroSensor.js' });
...
async function main() {
    ...
    logger.info({ message: 'Sensor iniciado: otroSensor.js' });
