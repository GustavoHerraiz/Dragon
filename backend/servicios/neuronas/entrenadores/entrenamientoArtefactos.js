...
let trainingData = [];
try {
    const rawData = fs.readFileSync("./datosRedArtefactos.json", "utf8");
...
function entrenarModelo() {
    ...
    configuraciones.forEach((config, index) => {
        ...
        fs.appendFileSync("./logs/trainingMetricsArtefactos.log", `Configuración ${index + 1}:\nError final: ${resultado.error}\n\n`);
    });

    // Guardar el modelo entrenado con la estructura correcta
    const modeloEntrenado = red.toJSON();
    fs.writeFileSync("./RedDeArtefactos_Entrenada.json", JSON.stringify(modeloEntrenado, null, 2));
    console.log("✅ Red entrenada guardada como RedDeArtefactos_Entrenada.json");
}
