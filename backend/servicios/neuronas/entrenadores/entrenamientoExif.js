...
import RedAnalisisExif from "./analisisDeExif.js";
import fs from "fs";

const { Trainer } = synaptic;

// Instancia de la red desde analisisDeExif.js
const red = new RedAnalisisExif().network; // Accedemos directamente a la red neuronal

// Cargar datos de entrenamiento
let trainingData = [];
try {
    const rawData = fs.readFileSync("./datosExif.json");
...
function entrenarModelo() {
    ...
    fs.writeFileSync("./RedDeExif_Entrenada.json", JSON.stringify(redEntrenada, null, 2));
    console.log("✅ Red entrenada guardada en RedDeExif_Entrenada.json");
}
